...
def render_page(p):
    ...
    if kurve_data:
        ...
        # Byg legende-items
        legend_items = f'<span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;color:#555"><span style="display:inline-block;width:18px;height:3px;background:#C8102E;border-radius:2px"></span>Kampe</span>'
        if has_maal:
            legend_items += f' <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;color:#555"><span style="display:inline-block;width:18px;height:3px;background:#1565C0;border-radius:2px"></span>Mål</span>'
        if has_sl:
            legend_items += f' <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;color:#555"><span style="display:inline-block;width:18px;height:3px;background:#2E7D32;border-radius:2px;border-top:2px dashed #2E7D32;height:0"></span>EM/VM</span>'
    ...
    <div style="display:flex;gap:12px;margin-top:8px;flex-wrap:wrap;align-items:center">
      {legend_items}
      <span style="margin-left:auto;font-size:11px;color:#bbb">alder →</span>
      <a href="/landsholdskurven/" style="font-size:11px;color:#C8102E">Sammenlign spillere ↗</a>
    </div>
    <script>
    (function(){{
      const KURVE    = {k_json};
      ...
      const HAS_MAAL = {'true' if has_maal else 'false'};
      const HAS_SL   = {'true' if has_sl else 'false'};

      const canvas = document.getElementById('kk-canvas');
      const ctx = canvas.getContext('2d');

      function drawLine(kurve, color, dashed, maxY, xOf, yOf) {{
        ...
        kurve.forEach((v, i) => {{
          const x = xOf(i), y = yOf(v, maxY);
          i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        }});
        ctx.stroke();
        ctx.setLineDash([]);
        // Slutpunkt
        const lx = xOf(kurve.length - 1), ly = yOf(kurve[kurve.length-1], maxY);
      ...
      function draw() {{
        ...
        function xOf(i, len) {{
          const l = len !== undefined ? len : KURVE.length;
          return PAD.left + i / (l - 1 || 1) * cW;
        }}
        function yOf(v, my) {{ return PAD.top + cH - (v / (my||1)) * cH; }}
        function xOfK(i, k) {{ return PAD.left + i / (k.length - 1 || 1) * cW; }}
        ...
        for (let i = 0; i < KURVE.length; i++) {{
          const age = 15 + i;
          if (age % 5 === 0) ctx.fillText(age, xOfK(i, KURVE), H - PAD.bottom + 12);
        }}

        // Kurver — alle på samme Y-akse (max = KURVE max)
        drawLine(KURVE,    '#C8102E', false, maxY, i => xOfK(i, KURVE),    (v,m) => yOf(v,m));
        if (HAS_MAAL) drawLine(KURVE_M,  '#1565C0', false, maxY, i => xOfK(i, KURVE_M),  (v,m) => yOf(v,m));
        if (HAS_SL)   drawLine(KURVE_SL, '#2E7D32', true,  maxY, i => xOfK(i, KURVE_SL), (v,m) => yOf(v,m));
      }}
